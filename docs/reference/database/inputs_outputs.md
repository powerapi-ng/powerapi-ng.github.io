...
Different storage options are available for [Sensors](/reference/overview/#sensor) and [Formulas](/reference/overview#formula).  

Storage is needed to save reports produced by each components.

- Sensors store their usage reports (raw metrics).  
- Formulas retrieve usage reports and store power consumption reports.  
- Individuals need to access reports for analysis, e.g., via visualization tools.   
